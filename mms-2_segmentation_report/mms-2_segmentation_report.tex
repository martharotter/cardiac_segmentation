...
\begin{document}
  ...
  \textbf{Purpose}:  To develop and evaluate multiple deep learning architectures for automated cardiac segmentation and pathology classification in cardiac MRI, addressing the need for robust, accurate, and reproducible cardiac functional assessment across multiple pathological conditions as per the M\&Ms-2 challenge guidelines.
  
  \textbf{Methods}: We implemented and compared three distinct deep learning approaches: (1) a temporal-aware 2D+Time UNet architecture using a 5-frame temporal context windows from CINE MRI sequences for enhanced cardiac motion understanding, (2) a dual-stage transformer pipeline combining SegFormer-B0 for 4-class semantic segmentation with Vision Transformer (ViT-B/16) for disease classification, and (3) traditional 2D UNet baselines. The temporal UNet approach leverages cardiac motion patterns across time frames, while the transformer pipeline processes individual 256×256 slices with SegFormer performing multi-class segmentation, followed by ViT pathology classification on concatenated segmented regions. Preprocessing involved temporal sequence alignment for the 2D+Time approach and standardized intensity normalization across all methods. Models were trained using the M\&Ms-2 dataset with appropriate train-validation splits, employing combined Dice and CrossEntropy losses for segmentation tasks and AdamW optimization.
    
  \textbf{Results}: Our evaluation identified distinct advantages for each approach: the 2D+Time UNet demonstrated superior segmentation accuracy by incorporating temporal cardiac motion context, achieving improved boundary delineation and anatomical consistency across cardiac cycles. The SegFormer-ViT pipeline showed robust performance on multi-center, multi-pathology data with effective handling of intensity variations and reliable pathology classification based on segmented cardiac features. Traditional 2D methods provided baseline performance benchmarks. All approaches successfully handled the challenges of data variability and diverse pathological presentations in the M\&Ms-2 dataset.
    
  \textbf{Conclusions}: This comprehensive study establishes multiple viable deep learning frameworks for automated cardiac MRI analysis, each with distinct advantages: temporal methods excel at motion-aware segmentation, transformer approaches provide robust multi-pathology classification, and the comparative framework enables optimal method selection based on specific clinical requirements. The temporal-aware and transformer-based approaches overcome limitations of traditional 2D methods and provide a foundation for clinical tools supporting standardized cardiac assessment across diverse conditions.
  \end{abstract}
  ...
  \textbf{Temporal Approaches}: Several studies have explored temporal modeling in cardiac segmentation. Guo et al. (2021) demonstrated the ability to achieve higher accuracy compared to the state-of-the-art methods with their spatial-sequential CNN network for temporal left ventricle segmentation\cite{guo2021}. However, these approaches often require substantial computational resources and careful temporal alignment.
  ...
  Future research should explore hybrid architectures that combine the temporal awareness of sequence-based models with the robust feature representation of transformer architectures. Investigation of more efficient attention mechanisms and temporal modeling approaches could address current computational limitations while preserving performance advantages.
